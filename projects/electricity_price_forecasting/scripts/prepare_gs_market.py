#!/usr/bin/env python3
"""Prepare GS(1).csv as a Diff-DoRA-compatible multi-node market dataset.

The script only repairs missing values. It intentionally keeps high prices and
large operational spikes because those may be valid market/dispatch states.
"""
...
from src.data.load_gs_market import load_gs_market

FREQ = "15min"
DEFAULT_SOURCE = Path("data/GS(1).csv")
DEFAULT_OUTPUT = Path("data/raw/gs_market")
DEFAULT_PROCESSED = Path("data/processed/gs_market.pkl")
DATE_COL = "Date"
...
NODE_META = {
    ...
    "发电总出力预测": {
        "english_name": "total_generation_forecast",
        "zone_type": "Supply",
        "type": "generation_forecast",
        "expert_id": 0,
        "unit": "MW",
        "description": "forecasted total generation output",
    },
    "竞价空间": {
        "english_name": "bidding_space",
        "zone_type": "Market",
        "type": "market_margin",
        "expert_id": 0,
        "unit": "MW",
        "description": "available bidding space / market margin",
    },
    "统一负荷预测": {
        "english_name": "unified_load_forecast",
        "zone_type": "Demand",
        "type": "load_forecast",
        "expert_id": 1,
        "unit": "MW",
        "description": "unified load forecast",
    },
    "抽蓄": {
        "english_name": "pumped_storage",
        "zone_type": "Storage",
        "type": "storage_dispatch",
        "expert_id": 1,
        "unit": "MW",
        "description": "pumped-storage dispatch signal",
    },
    "统一新能源预测": {
        "english_name": "unified_renewable_forecast",
        "zone_type": "Renewable",
        "type": "renewable_forecast",
        "expert_id": 0,
        "unit": "MW",
        "description": "unified renewable generation forecast",
    },
    "联络线计划": {
        "english_name": "tie_line_schedule",
        "zone_type": "Interconnection",
        "type": "tie_line_schedule",
        "expert_id": 1,
        "unit": "MW",
        "description": "tie-line schedule / interconnection exchange plan",
    },
}
...
def _build_adjacency(values: pd.DataFrame, train_idx: np.ndarray) -> pd.DataFrame:
    train_values = values.iloc[train_idx]
    corr = train_values.corr().abs().replace([np.inf, -np.inf], np.nan).fillna(0.0)
    arr = corr.to_numpy(dtype=float, copy=True)
    np.fill_diagonal(arr, 0.0)
    corr = pd.DataFrame(arr, index=values.columns, columns=values.columns)
...
def _split_indices_2025_internal(index: pd.DatetimeIndex) -> dict[str, np.ndarray]:
    year_positions = np.flatnonzero(index.year == 2025)
...
def _write_node_meta(values: pd.DataFrame, output_path: Path) -> pd.DataFrame:
    rows = []
    for col in values.columns:
        meta = dict(NODE_META[col])
        rows.append(
            {
                "node_id": col,
                ...
                "capacity": float(np.nanmax(np.abs(values[col].to_numpy(dtype=float)))),
                "area": "GS",
                "unit": meta["unit"],
                "source_column": col,
    ...
    node_meta = pd.DataFrame(rows)
    node_meta.to_csv(output_path, index=False)
    return node_meta
...
def prepare(
    ...
    *,
    ...
    missing_columns = [col for col in [DATE_COL, *NODE_COLUMNS] if col not in df.columns]
    if missing_columns:
        raise ValueError(f"Missing required columns: {missing_columns}")

    timestamps, date_note = _regular_datetime(df[DATE_COL])
    values = df[NODE_COLUMNS].apply(pd.to_numeric, errors="coerce")
    ...
    repaired.reset_index().to_csv(occupancy_path, index=False)
    ...
    repaired[["Price"]].reset_index().to_csv(price_path, index=False)
    _write_node_meta(repaired, nodes_path)
    _build_adjacency(repaired, split_indices["train"]).to_csv(adjacency_path, index=False)
