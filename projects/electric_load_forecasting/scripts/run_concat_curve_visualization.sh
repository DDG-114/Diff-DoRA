...
OUTPUT_DIR="${RUN_ROOT}/concat_curves"
...
if nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits | grep -q '[0-9]'; then
  echo "Detected active GPU compute processes. Please free both GPUs before starting concat-curve visualization." >&2
...
FULL_JSON="${OUTPUT_DIR}/full_concat_gpu0.json"
WO_JSON="${OUTPUT_DIR}/wo_concat_gpu1.json"

CUDA_VISIBLE_DEVICES=0 python -m src.analysis.eval_concat_windows_variant \
  --dataset "${DATASET}" \
  --horizon "${HORIZON}" \
  --use_rag \
  --use_diff_dora \
...
CUDA_VISIBLE_DEVICES=1 python -m src.analysis.eval_concat_windows_variant \
  --dataset "${DATASET}" \
  --horizon "${HORIZON}" \
  --use_rag \
  --prompt_style cot \
...
python -m src.analysis.visualize_concat_curves \
  --full-json "${FULL_JSON}" \
  --wo-json "${WO_JSON}" \
  --output-dir "${OUTPUT_DIR}"

echo "Concat-curve visualization complete:"
echo "  ${OUTPUT_DIR}/cbd_concat_curve.svg"
echo "  ${OUTPUT_DIR}/residential_concat_curve.svg"
echo "  ${OUTPUT_DIR}/concat_curve_summary.json"
