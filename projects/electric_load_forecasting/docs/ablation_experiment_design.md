...
### 12.2 Legacy Diff-DoRA 控制器容量消融

目标：仅用于回看旧版 controller-gated Diff-DoRA 是否过强或过弱。

说明：当前论文对齐实现已经切换为 prompt-only Diff-DoRA，这一节不再适用于主线复现实验。
...
旧仓库曾经暴露 `diff_hidden_dim` 和 `diff_scale`；当前主线实现已移除这两个参数。
